...
const projects = [
  ...
  },
  {
    ...
    subtitle: 'High-Throughput Go Pub/Sub Server',
    description: 'High-throughput Go pub/sub server achieving 1.54M msg/sec via goroutine-per-subscription fan-out, an RWMutex-guarded topic registry, and per-API-key token bucket rate limiting; at-least-once delivery with DLQ after retry exhaustion, race-detector clean under concurrent load. Dual-plane architecture (REST control, WebSocket data) with Bearer auth, PostgreSQL persistence behind a store interface, and Prometheus metrics; ships a Go client SDK mirroring cloud.google.com/go/pubsub for drop-in migration via config change.',
    accent: '#569e71',
    accentRgb: '86,158,113',
    stats: [
      { value: '1.54M/s',  label: 'msg throughput' },
      { value: 'race-safe', label: 'concurrent load' },
      { value: 'DLQ',      label: 'retry exhaustion' },
      { value: 'REST+WS',  label: 'dual-plane' },
    ],
    stack: [
      ...
      { name: 'WebSocket',   category: 'stream' },
      { name: 'Prometheus',  category: 'infra' },
    ],
    metadata: [
      { label: 'ARCH',     value: 'Dual-plane: REST control + WebSocket data' },
      { label: 'CONCURR',  value: 'Goroutine-per-subscription fan-out, RWMutex registry' },
      { label: 'DELIVERY', value: 'At-least-once, token bucket rate limit, DLQ after retries' },
      { label: 'SDK',      value: 'Go client SDK mirrors cloud.google.com/go/pubsub' },
    ],
  ...
  },
  {
    id: 'PRJ-11',
    title: 'CHRONOS_V2',
    subtitle: 'Time-Series Metrics API on TimescaleDB',
    description: 'Time-series metrics API built on TimescaleDB with hypertable partitioning, 7-day chunk compression, and continuous aggregates for fast rollups. FastAPI async endpoints serve WebSocket and SSE feeds backed by asyncpg connection pooling and a multi-tier Redis cache (60s/300s TTL). JWT RS256 authentication with Redis-backed refresh token rotation and fine-grained RBAC roles, validated by 80 passing integration tests covering auth flows, data access, and aggregation correctness.',
    accent: '#cc8258',
    accentRgb: '204,130,88',
    stats: [
      { value: '80',        label: 'integration tests' },
      { value: '7-day',     label: 'chunk compression' },
      { value: '60s/300s',  label: 'redis TTL tiers' },
      { value: 'RS256',     label: 'JWT auth' },
    ],
    stack: [
      { name: 'Python',      category: 'lang' },
      { name: 'FastAPI',     category: 'api' },
      { name: 'TimescaleDB', category: 'database' },
      { name: 'asyncpg',     category: 'database' },
      { name: 'Redis',       category: 'database' },
      { name: 'JWT RS256',   category: 'infra' },
    ],
    metadata: [
      { label: 'ARCH',  value: 'TimescaleDB hypertables + continuous aggregates' },
      { label: 'CACHE', value: 'Multi-tier Redis (60s/300s TTL)' },
      { label: 'AUTH',  value: 'JWT RS256 + Redis-backed refresh rotation' },
      { label: 'TEST',  value: '80 passing integration tests (auth, access, aggregation)' },
    ],
  },
  {
    id: 'PRJ-04',
